...
celltypes <- c(sort(c(grep("^M", unique(seurat_object$celltype), value = T), c("Neut1"))),
               sort(grep("^L", unique(seurat_object$celltype), value = T)), c("NK", "Treg", "B1", "B2"), c("DC", "NOS1"))
...
netVisual_bubble(CC_merged_object, sources.use = 20, targets.use = grep("^L", levels(CC_objects_compare[[i]]@idents)), thresh = 1, comparison = c(2, 1), angle.x = 45)
