...
create_dotplot_heatmap <- function(seurat_object, plot_features, group_var, group_colors, column_title, km=5, row.order = NULL){
    ...
    col_fun = circlize::colorRamp2(c(-1, 2), c("azure", "tomato3"))
...
create_dotplot_heatmap_horizontal <- function(seurat_object, plot_features, group_var, group_colors, column_title, km=5){
    ...
    cluster_anno <- colnames(exp_mat)
    dotplot_col <- list("cluster" = group_colors)
    column_ha <- rowAnnotation( # HeatmapAnnotation / rowAnnotation
        cluster = cluster_anno,
        col = dotplot_col,
        na_col = "grey"
    )
