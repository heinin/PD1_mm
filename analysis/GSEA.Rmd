...
### Significance testing across all celltypes
...
### Barplot of differentially regulated pathways across all cell types

```{r, fig.width=14, fig.height=8}

sig_res %>%
  mutate(delta = median.NEO_16 - median.CTRL_16,
         sign = sign(delta),
...
sig_res %>%
  mutate(delta = median.NEO_16 - median.ADJ_16,
         sign = sign(delta),
  ...
  ggplot(aes(x = delta, y = reorder(pathways, delta), fill = signstr)) +
    ...
    ylab("") +
    xlab(expression(Delta ~ "median enrichment score")) +
    ggtitle("NEO Day 16 vs. ADJ Day 16")

sig_res %>%
  mutate(delta = median.CTRL_16 - median.ADJ_16,
         sign = sign(delta),
         signstr = if_else(sign == 1, "CTRL_16", "ADJ_16")) %>%
  ggplot(aes(x = delta, y = reorder(pathways, delta), fill = signstr)) +
    geom_bar(stat = "identity") +
    #geom_col(width = 0.85) +
    scale_fill_manual(values = c("deeppink3", "aquamarine3")) +
    ...
    ylab("") +
    xlab(expression(Delta ~ "median enrichment score")) +
    ggtitle("CTRL Day 16 vs. ADJ Day 16")

sig_res %>%
  mutate(delta = median.NEO_12 - median.CTRL_12,
         sign = sign(delta),
         signstr = if_else(sign == 1, "NEO_12", "CTRL_12")) %>%
  ggplot(aes(x = delta, y = reorder(pathways, delta), fill = signstr)) +
    geom_bar(stat = "identity") +
    #geom_col(width = 0.85) +
    scale_fill_manual(values = c("aquamarine3", "deeppink3")) +
    theme_classic() +
    #manuscript_theme +
    ylab("") +
    xlab(expression(Delta ~ "median enrichment score")) +
...
myeloid <- subset(seurat_data, subset = celltype %in% c("Neut1", paste0("M", seq(1, 7))))

res_myel <- myeloid@meta.data %>%
  dplyr::select(c("orig.ident", "Treatment_Day", colnames(res)))

sig_res_myel <- data.frame(getSignificance(res_myel,
                                           group = "Treatment_Day",
                                           fit = "ANOVA"))
sig_res_myel$pathways <- rownames(sig_res_myel)
sig_res_myel <- sig_res_myel %>% filter(FDR < 0.05) %>% arrange(FDR)

dim(sig_res_myel)

sig_res_myel %>% dplyr::select(c("pathways"))

```

### Barplot of differentially regulated pathways across myeloid cell types  (M1-7, Neut)
...
sig_res_myel %>%
  ...
  ggplot(aes(x = delta, y = reorder(pathways, delta), fill = signstr)) +
    ...
    ylab("") +
    xlab(expression(Delta ~ "median enrichment score")) +
    ggtitle("CTRL Day 16 vs. ADJ Day 16, myeloid")
...
lymphoid <- subset(seurat_data, subset = celltype %in% paste0("M", seq(1, 7)))

res_lymp <- lymphoid@meta.data %>%
  dplyr::select(c("orig.ident", "Treatment_Day", colnames(res)))

sig_res_lymp <- data.frame(getSignificance(res_lymp,
                                           group = "Treatment_Day",
                                           fit = "ANOVA"))
sig_res_lymp$pathways <- rownames(sig_res_lymp)
sig_res_lymp <- sig_res_lymp %>% filter(FDR < 0.05) %>% arrange(FDR)

dim(sig_res_lymp)

sig_res_lymp %>% dplyr::select(c("pathways"))

```

### Barplot of differentially regulated pathways across lymphoid cell types (L1-6, Treg, NK)
...
sig_res_lymp %>%
  ...
  ggplot(aes(x = delta, y = reorder(pathways, delta), fill = signstr)) +
    ...
    ylab("") +
    xlab(expression(Delta ~ "median enrichment score")) +
    ggtitle("CTRL Day 16 vs. ADJ Day 16, lymphoid")
...
sig_res_lymp %>%
  ...
  ggplot(aes(x = delta, y = reorder(pathways, delta), fill = signstr)) +
    ...
    ylab("") +
    xlab(expression(Delta ~ "median enrichment score")) +
    ggtitle("NEO Day 12 vs. CTRL Day 12, lymphoid")
